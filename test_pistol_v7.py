"""
Tactical Combat Pistol V7 - Master Quality Procedural Firearm
Engineered to achieve maximum fidelity matching the concept art:
- Aggressive 20° tactical grip rake with deep beavertail and contoured finger grooves
- Ergonomic thumb rest indents on frame
- Two-tone contrast: Slate graphite CNC slide + deep matte polymer receiver
- 45° angled front dust cover matching slide muzzle profile
- Deep angled chevron cocking serrations (front & rear) catching rim light
- Threaded barrel with knurled cap & crown
- Night sight dots with emissive glow
- Studio beauty render matching concept art
"""
...
def create_materials():
    ...
    bsdf.inputs['Base Color'].default_value = (0.075, 0.080, 0.088, 1.0) # Slate graphite
...
def build_unified_frame(materials):
    ...
    pts = [
        (+0.082, 0.072),  # 0: Front top of dust cover
        (+0.074, 0.050),  # 1: 45° angled front snout cut
        (+0.035, 0.049),  # 2: Picatinny rail rear limit
    ...
    for side in [-1, 1]:
        bpy.ops.mesh.primitive_cylinder_add(
            ...
            radius=0.009,
            depth=0.006,
            location=(side * 0.0135, -0.006, 0.054),
...
def build_slide_and_barrel(materials):
    ...
    # 5. Angled Chevron Cocking Serrations (Deep tactical grooves, 25° forward rake)
    # Rear Serrations
    for i in range(6):
        ...
        for side in [-1, 1]:
            bpy.ops.mesh.primitive_cube_add(
                ...
                scale=(0.0032, 0.0024, height * 0.68),
...
def setup_studio_lighting():
    ...
    # 1. Key Light (Soft, broad overhead softbox at 45°)
...
def render_studio_views(output_dir):
    ...
    cam_obj.location = (-0.38, 0.28, 0.16)
...
def main():
    ...
    print("[COMPLETE] Tactical Pistol V7 generation and rendering finished successfully!")
