"""
Tactical Combat Pistol V6 - Production High Fidelity Model
Matches concept art with precision:
- Solid manifold ergonomic polymer frame with contoured finger grooves, swept beavertail, and flared magwell
- 13mm narrowed combat trigger guard with finger rest
- CNC machined slide with 45° top chamfers, front bull-nose taper, angled chevron serrations, and flush ejection port
- Threaded tactical barrel with knurled thread cap
- 3-dot combat iron sights
- Dual-tone high-grade PBR shaders (matte nitride steel & textured polymer)
- Heroic 3/4 front beauty angle & studio lighting
"""
...
def create_materials():
    ...
    bsdf.inputs['Base Color'].default_value = (0.04, 0.043, 0.047, 1.0)
    bsdf.inputs['Metallic'].default_value = 0.90
    bsdf.inputs['Roughness'].default_value = 0.32
    links.new(bsdf.outputs['BSDF'], out_node.inputs['Surface'])
    
    # 2. Polymer Frame (Matte dark grey with subtle roughness)
...
def build_unified_frame(materials):
    """
    Creates the solid ergonomic lower frame with seamless trigger guard,
    finger grooves, flared magwell, and beavertail.
    """
    curve_data = bpy.data.curves.new('PistolFrameCurve', type='CURVE')
    curve_data.dimensions = '2D'
    curve_data.extrude = 0.0135 # 27mm total frame width
    curve_data.bevel_depth = 0.0 # Keep 0 so caps fill completely as a solid volume!
    ...
    for side in [-1, 1]:
        bpy.ops.mesh.primitive_cube_add(
            size=1.0,
            location=(side * 0.011, 0.006, 0.036),
            scale=(0.010, 0.048, 0.032)
        )
    ...
    # 3. Picatinny Rail Slots under dust cover
    for r in range(4):
        slot_y = 0.045 + r * 0.010
        bpy.ops.mesh.primitive_cube_add(
            size=1.0,
            location=(0.0, slot_y, 0.050),
            scale=(0.032, 0.0048, 0.004)
        )
    ...
    # 4. Tactical Grip Stippling Panels (Left & Right side inserts)
    for side in [-1, 1]:
        bpy.ops.mesh.primitive_cube_add(
            size=1.0,
    ...
    # 6. Realistic Trigger with Safety Blade
    ...
    # 7. Slide Stop Lever & Takedown Latch
    bpy.ops.mesh.primitive_cube_add(
        size=1.0,
...
def setup_studio_lighting():
    ...
    bpy.ops.mesh.primitive_plane_add(
        size=10.0,
        location=(0.0, 0.0, -0.063) # Under magazine basepad
    )
    table = bpy.context.active_object
    table.name = "Studio_Table"
    table.data.materials.append(mat_table)

    # 1. Key Light (Soft, large 45° overhead warm-white area light)
    ...
    # 3. Rim / Edge Strip Light (creates high-contrast highlight along slide and sights)
...
def render_studio_views(output_dir):
    ...
    - preview_perspective.png (Heroic 3/4 beauty view matching concept art!)
    ...
    target.location = (0.0, 0.012, 0.035)
    ...
    cam_obj.location = (-0.38, 0.30, 0.17)
    ...
    # 2. Side View
    cam_obj.location = (-0.52, 0.012, 0.035)
    bpy.context.view_layer.update()
...
def main():
    ...
    print("[COMPLETE] Tactical Pistol V6 generation and rendering finished successfully!")
